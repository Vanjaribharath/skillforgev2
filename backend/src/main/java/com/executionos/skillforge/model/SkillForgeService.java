...
public class SkillForgeService {
    ...
    public LoginResponse login(String email, String rawPassword) {
        ...
        SfUser matched = candidates.stream()
                .filter(u -> u.getPasswordHash() != null && passwordEncoder.matches(rawPassword, u.getPasswordHash()))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Invalid email or password"));
        if (matched.getStatus() != UserStatus.ACTIVE) {
            throw new IllegalArgumentException("Account is not active");
        }
        String token = jwtService.issueAccessToken(matched.getEmail(), matched.getId(), matched.getRole().name(), matched.getOrganizationId());
        return new LoginResponse(token, userResponse(matched));
    }
    ...
    public LoginResponse googleLogin(String idToken) {
        ...
        String token = jwtService.issueAccessToken(user.getEmail(), user.getId(), user.getRole().name(), user.getOrganizationId());
        return new LoginResponse(token, userResponse(user));
    }
