...
public final class ResourceControllers {
    ...
    public static class ReadModelController {
        // NOTE: every endpoint in this controller returns a hardcoded
        // placeholder, not a real aggregate query against the database.
        // Flagged explicitly per the instruction not to present fake data
        // as if it were real -- see MERGE_AND_FIX_REPORT.md. Building real
        // versions of these (actual task-completion counts, actual focus
        // totals, an actual search index) is real feature work, not a
        // wiring fix, and is out of scope for this pass.
        @GetMapping("/dashboard/today") Object today() { return Map.of("tasksDone", 5, "tasksTotal", 8, "focusMinutes", 145, "nextBlock", "Build vault search", "isPlaceholder", true); }
        @GetMapping("/dashboard/stats") Object dashboardStats() { return Map.of("weeklyFocusHours", 18, "streak", 12, "notes", 34, "isPlaceholder", true); }
        @GetMapping("/dashboard/streaks") Object streaks() { return Map.of("current", 12, "best", 29, "isPlaceholder", true); }
        @GetMapping("/analytics/weekly") Object weekly() { return Map.of("days", new int[]{3, 5, 4, 6, 2, 7, 4}, "isPlaceholder", true); }
        @GetMapping("/analytics/categories") Object categories() { return Map.of("code", 42, "learning", 25, "shipping", 33, "isPlaceholder", true); }
        @GetMapping("/analytics/deep-work") Object deepWork() { return Map.of("hours", 18, "averageSessionMinutes", 52, "isPlaceholder", true); }
        // Also a stub -- always returns zero results regardless of query.
        @GetMapping("/search") Object search(@RequestParam String q, @RequestParam(required = false) String type) { return Map.of("query", q, "type", type, "results", new Object[]{}, "isPlaceholder", true); }
    }
