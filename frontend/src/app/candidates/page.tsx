...
import { FileSpreadsheet, MailPlus, Plus, TrendingUp, UsersRound, X } from "lucide-react";
...
export default function CandidatesPage() {
  ...
  const addCandidateMutation = useMutation({
    ...
    onError: (error: any) => {
      setMessage(`Failed to add candidate: ${error.response?.data?.message || error.message}`);
    }
  });
      ...
      <Card>
        ...
        <div className="grid gap-3">
          {candidates.map((candidate: any) => (
            <div key={candidate.id} className="grid gap-3 rounded-md border border-line p-3 md:grid-cols-[1.2fr_1fr_1fr_auto] md:items-center">
              <div>
              ...
              <div className="flex items-center gap-2 text-sm font-semibold text-green"><TrendingUp size={17} /> {candidate.status === 'ACTIVE' ? "+12%" : "N/A"}</div>
            </div>
          ))}
          {candidates.length === 0 && (
